"""Phase 2 card P2-63 gate for Linux CI workflow Linux-validation terminal publish.

This script consumes the P2-62 Linux-validation verdict publish artifact and
converges one terminal publish contract for Linux validation:
1) validate Linux validation verdict publish + evidence consistency,
...
ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_STATUSES: set[str] = {
    "published",
    "published_with_follow_up",
    "blocked",
    "contract_failed",
}
ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_DECISIONS: set[str] = {
    "announce_linux_validation_passed",
    "announce_linux_validation_passed_with_follow_up",
    "announce_linux_validation_blocker",
    "abort_publish",
}
ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_CHANNELS: set[str] = {
    "release",
    "follow_up",
    "blocker",
}
...
def load_linux_validation_verdict_publish_report(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError(f"{path}: Linux validation verdict publish report not found")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"{path}: invalid JSON ({exc})") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"{path}: Linux validation verdict publish payload must be object")
    ...
    verdict_publish_status = _coerce_str(
        payload["linux_validation_verdict_publish_status"],
        field="linux_validation_verdict_publish_status",
        path=path,
    )
    if verdict_publish_status not in ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_STATUSES:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_publish_status' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_STATUSES)}"
        )

    verdict_publish_decision = _coerce_str(
        payload["linux_validation_verdict_publish_decision"],
        field="linux_validation_verdict_publish_decision",
        path=path,
    )
    if verdict_publish_decision not in ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_DECISIONS:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_publish_decision' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_DECISIONS)}"
        )

    verdict_publish_exit_code = _coerce_int(
        payload["linux_validation_verdict_publish_exit_code"],
        field="linux_validation_verdict_publish_exit_code",
        path=path,
    )
    if verdict_publish_exit_code < 0:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_publish_exit_code' must be >= 0"
        )

    verdict_publish_channel = _coerce_str(
        payload["linux_validation_verdict_publish_channel"],
        field="linux_validation_verdict_publish_channel",
        path=path,
    )
    if verdict_publish_channel not in ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_CHANNELS:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_publish_channel' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_CHANNELS)}"
        )
    ...
    return {
        ...
        "linux_validation_verdict_publish_status": verdict_publish_status,
        "linux_validation_verdict_publish_decision": verdict_publish_decision,
        "linux_validation_verdict_publish_exit_code": verdict_publish_exit_code,
        ...
        "linux_validation_verdict_publish_channel": verdict_publish_channel,
...
def build_linux_validation_terminal_publish_payload(
    linux_validation_verdict_publish_report: dict[str, Any], *, source_path: Path
) -> dict[str, Any]:
    verdict_publish_status = str(
        linux_validation_verdict_publish_report["linux_validation_verdict_publish_status"]
    )
    verdict_publish_decision = str(
        linux_validation_verdict_publish_report["linux_validation_verdict_publish_decision"]
    )
    verdict_publish_exit_code = int(
        linux_validation_verdict_publish_report["linux_validation_verdict_publish_exit_code"]
    )
    verdict_publish_should_notify = bool(
        linux_validation_verdict_publish_report["linux_validation_verdict_publish_should_notify"]
    )
    verdict_publish_requires_manual_action = bool(
        linux_validation_verdict_publish_report[
            "linux_validation_verdict_publish_requires_manual_action"
        ]
    )
    verdict_publish_channel = str(
        linux_validation_verdict_publish_report["linux_validation_verdict_publish_channel"]
    )

    reason_codes = list(linux_validation_verdict_publish_report["reason_codes"])
    structural_issues = list(linux_validation_verdict_publish_report["structural_issues"])
    missing_artifacts = list(linux_validation_verdict_publish_report["missing_artifacts"])
    evidence_manifest = list(linux_validation_verdict_publish_report["evidence_manifest"])
    ...
    if missing_evidence:
        ...
        reason_codes.append("linux_validation_terminal_publish_evidence_missing")

    expected_verdict_publish = {
        "published": (
            "announce_linux_validation_passed",
            0,
            True,
    ...
    (
        ...
        expected_channel,
    ) = expected_verdict_publish[verdict_publish_status]

    if verdict_publish_decision != expected_decision:
        structural_issues.append("linux_validation_verdict_publish_decision_mismatch")
    if verdict_publish_should_notify != expected_should_notify:
        structural_issues.append("linux_validation_verdict_publish_should_notify_mismatch")
    if verdict_publish_requires_manual_action != expected_requires_manual_action:
        structural_issues.append(
            "linux_validation_verdict_publish_requires_manual_action_mismatch"
        )
    if verdict_publish_channel != expected_channel:
        structural_issues.append("linux_validation_verdict_publish_channel_mismatch")
    if expected_exit_code == 0 and verdict_publish_exit_code != 0:
        structural_issues.append("linux_validation_verdict_publish_exit_code_mismatch_published")
    if (
        expected_exit_code is None
        and verdict_publish_exit_code == 0
        and verdict_publish_status != "published"
    ):
        structural_issues.append(
            "linux_validation_verdict_publish_exit_code_mismatch_non_published"
        )
    ...
    if structural_issues or missing_artifacts:
        ...
        terminal_publish_decision = "abort_terminal_publish"
        terminal_publish_exit_code = max(1, verdict_publish_exit_code)
    ...
    elif verdict_publish_status == "published":
        terminal_publish_status = "terminal_published"
        terminal_publish_decision = "announce_linux_validation_terminal_passed"
    ...
    elif verdict_publish_status == "published_with_follow_up":
        terminal_publish_status = "terminal_published_with_follow_up"
        terminal_publish_decision = "announce_linux_validation_terminal_passed_with_follow_up"
        terminal_publish_exit_code = max(1, verdict_publish_exit_code)
        terminal_publish_should_notify = True
        terminal_publish_ready_for_handoff = True
        terminal_publish_requires_manual_action = False
        terminal_publish_channel = "follow_up"
        reason_codes.append("linux_validation_terminal_publish_terminal_published_with_follow_up")
    elif verdict_publish_status == "blocked":
        terminal_publish_status = "terminal_blocked"
        terminal_publish_decision = "announce_linux_validation_terminal_blocker"
        terminal_publish_exit_code = max(1, verdict_publish_exit_code)
        terminal_publish_should_notify = True
        terminal_publish_ready_for_handoff = False
        terminal_publish_requires_manual_action = True
        terminal_publish_channel = "blocker"
        reason_codes.append("linux_validation_terminal_publish_terminal_blocked")
    else:
        terminal_publish_status = "terminal_contract_failed"
        terminal_publish_decision = "abort_terminal_publish"
        terminal_publish_exit_code = max(1, verdict_publish_exit_code)
        ...
        terminal_publish_requires_manual_action = True
        terminal_publish_channel = "blocker"
        reason_codes.append("linux_validation_terminal_publish_upstream_contract_failed")

    if terminal_publish_status not in ALLOWED_LINUX_VALIDATION_TERMINAL_PUBLISH_STATUSES:
        raise ValueError("internal: unsupported linux_validation_terminal_publish_status computed")
    if terminal_publish_decision not in ALLOWED_LINUX_VALIDATION_TERMINAL_PUBLISH_DECISIONS:
        raise ValueError("internal: unsupported linux_validation_terminal_publish_decision computed")
    if terminal_publish_channel not in ALLOWED_LINUX_VALIDATION_TERMINAL_PUBLISH_CHANNELS:
        raise ValueError("internal: unsupported linux_validation_terminal_publish_channel computed")

    reason_codes = _unique(reason_codes)
    summary = (
        f"linux_validation_verdict_publish_status={verdict_publish_status} "
        f"linux_validation_terminal_publish_status={terminal_publish_status} "
        f"linux_validation_terminal_publish_decision={terminal_publish_decision}"
    )

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_linux_validation_verdict_publish_report": str(source_path),
        "source_linux_validation_verdict_report": str(
            linux_validation_verdict_publish_report["source_linux_validation_verdict_report"]
        ),
        "source_linux_validation_dispatch_report": str(
            linux_validation_verdict_publish_report["source_linux_validation_dispatch_report"]
        ),
        "project_root": str(linux_validation_verdict_publish_report["project_root"]),
        "source_terminal_verdict_report": str(
            linux_validation_verdict_publish_report["source_terminal_verdict_report"]
        ),
        "source_release_final_publish_archive_report": str(
            linux_validation_verdict_publish_report["source_release_final_publish_archive_report"]
        ),
        "source_gate_manifest_drift_report": str(
            linux_validation_verdict_publish_report["source_gate_manifest_drift_report"]
        ),
        "release_final_publish_archive_status": str(
            linux_validation_verdict_publish_report["release_final_publish_archive_status"]
        ),
        "gate_manifest_drift_status": str(
            linux_validation_verdict_publish_report["gate_manifest_drift_status"]
        ),
        "gate_manifest_drift_missing_runtime_tests": list(
            linux_validation_verdict_publish_report["gate_manifest_drift_missing_runtime_tests"]
        ),
        "gate_manifest_drift_missing_manifest_entries": list(
            linux_validation_verdict_publish_report["gate_manifest_drift_missing_manifest_entries"]
        ),
        "gate_manifest_drift_orphan_manifest_entries": list(
            linux_validation_verdict_publish_report["gate_manifest_drift_orphan_manifest_entries"]
        ),
        "terminal_verdict_status": str(
            linux_validation_verdict_publish_report["terminal_verdict_status"]
        ),
        "terminal_verdict_decision": str(
            linux_validation_verdict_publish_report["terminal_verdict_decision"]
        ),
        "terminal_verdict_exit_code": int(
            linux_validation_verdict_publish_report["terminal_verdict_exit_code"]
        ),
        "terminal_verdict_should_proceed": bool(
            linux_validation_verdict_publish_report["terminal_verdict_should_proceed"]
        ),
        "terminal_verdict_requires_manual_action": bool(
            linux_validation_verdict_publish_report["terminal_verdict_requires_manual_action"]
        ),
        "terminal_verdict_channel": str(
            linux_validation_verdict_publish_report["terminal_verdict_channel"]
        ),
        "linux_validation_should_dispatch": bool(
            linux_validation_verdict_publish_report["linux_validation_should_dispatch"]
        ),
        "linux_validation_requires_manual_action": bool(
            linux_validation_verdict_publish_report["linux_validation_requires_manual_action"]
        ),
        "linux_validation_channel": str(
            linux_validation_verdict_publish_report["linux_validation_channel"]
        ),
        "linux_validation_dispatch_status": str(
            linux_validation_verdict_publish_report["linux_validation_dispatch_status"]
        ),
        "linux_validation_dispatch_decision": str(
            linux_validation_verdict_publish_report["linux_validation_dispatch_decision"]
        ),
        "linux_validation_dispatch_exit_code": int(
            linux_validation_verdict_publish_report["linux_validation_dispatch_exit_code"]
        ),
        "linux_validation_dispatch_attempted": bool(
            linux_validation_verdict_publish_report["linux_validation_dispatch_attempted"]
        ),
        "linux_validation_dispatch_command_returncode": linux_validation_verdict_publish_report[
            "linux_validation_dispatch_command_returncode"
        ],
        "linux_validation_verdict_status": str(
            linux_validation_verdict_publish_report["linux_validation_verdict_status"]
        ),
        "linux_validation_verdict_decision": str(
            linux_validation_verdict_publish_report["linux_validation_verdict_decision"]
        ),
        "linux_validation_verdict_exit_code": int(
            linux_validation_verdict_publish_report["linux_validation_verdict_exit_code"]
        ),
        "linux_validation_passed": bool(
            linux_validation_verdict_publish_report["linux_validation_passed"]
        ),
        ...
        "linux_validation_verdict_channel": str(
            linux_validation_verdict_publish_report["linux_validation_verdict_channel"]
        ),
        "linux_validation_verdict_attempted": bool(
            linux_validation_verdict_publish_report["linux_validation_verdict_attempted"]
        ),
        "linux_validation_verdict_timeout_seconds": int(
            linux_validation_verdict_publish_report["linux_validation_verdict_timeout_seconds"]
        ),
        "linux_validation_verdict_command": str(
            linux_validation_verdict_publish_report["linux_validation_verdict_command"]
        ),
        "linux_validation_verdict_command_parts": list(
            linux_validation_verdict_publish_report["linux_validation_verdict_command_parts"]
        ),
        "dry_run": bool(linux_validation_verdict_publish_report["dry_run"]),
        "verdict_command_returncode": linux_validation_verdict_publish_report[
            "verdict_command_returncode"
        ],
        "verdict_command_stdout_tail": str(
            linux_validation_verdict_publish_report["verdict_command_stdout_tail"]
        ),
        "verdict_command_stderr_tail": str(
            linux_validation_verdict_publish_report["verdict_command_stderr_tail"]
        ),
        "verdict_error_type": str(
            linux_validation_verdict_publish_report["verdict_error_type"]
        ),
        "verdict_error_message": str(
            linux_validation_verdict_publish_report["verdict_error_message"]
        ),
        "release_run_id": linux_validation_verdict_publish_report["release_run_id"],
        "release_run_url": str(linux_validation_verdict_publish_report["release_run_url"]),
        "follow_up_queue_url": str(linux_validation_verdict_publish_report["follow_up_queue_url"]),
        "linux_validation_dispatch_summary": str(
            linux_validation_verdict_publish_report["linux_validation_dispatch_summary"]
        ),
        "linux_validation_verdict_summary": str(
            linux_validation_verdict_publish_report["linux_validation_verdict_summary"]
        ),
        "linux_validation_verdict_publish_status": verdict_publish_status,
        "linux_validation_verdict_publish_decision": verdict_publish_decision,
        "linux_validation_verdict_publish_exit_code": verdict_publish_exit_code,
        "linux_validation_verdict_publish_should_notify": verdict_publish_should_notify,
        "linux_validation_verdict_publish_requires_manual_action": verdict_publish_requires_manual_action,
        "linux_validation_verdict_publish_channel": verdict_publish_channel,
        "linux_validation_verdict_publish_summary": str(
            linux_validation_verdict_publish_report["linux_validation_verdict_publish_summary"]
        ),
        ...
        "linux_validation_terminal_publish_should_notify": terminal_publish_should_notify,
        "linux_validation_terminal_publish_ready_for_handoff": terminal_publish_ready_for_handoff,
        "linux_validation_terminal_publish_requires_manual_action": terminal_publish_requires_manual_action,
        "linux_validation_terminal_publish_channel": terminal_publish_channel,
        "linux_validation_terminal_publish_summary": summary,
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    lines = [
        "# Linux CI Workflow Linux Validation Terminal Publish Report",
        "",
        (
            "- Linux Validation Terminal Publish Status: "
            f"**{str(payload['linux_validation_terminal_publish_status']).upper()}**"
        ),
        (
            "- Linux Validation Terminal Publish Decision: "
            f"`{payload['linux_validation_terminal_publish_decision']}`"
        ),
        (
            "- Linux Validation Terminal Publish Exit Code: "
            f"`{payload['linux_validation_terminal_publish_exit_code']}`"
        ),
        (
            "- Linux Validation Terminal Publish Should Notify: "
            f"`{payload['linux_validation_terminal_publish_should_notify']}`"
        ),
        (
            "- Linux Validation Terminal Publish Ready For Handoff: "
            f"`{payload['linux_validation_terminal_publish_ready_for_handoff']}`"
        ),
        (
            "- Linux Validation Terminal Publish Requires Manual Action: "
            f"`{payload['linux_validation_terminal_publish_requires_manual_action']}`"
        ),
        (
        ...
        (
            "- Linux Validation Verdict Publish Status: "
            f"`{payload['linux_validation_verdict_publish_status']}`"
        ),
        ...
        "## Summary",
        "",
        f"- {payload['linux_validation_terminal_publish_summary']}",
        "",
        "## Reason Codes",
    ]
...
def build_github_output_values(
    ...
    return {
        "workflow_linux_validation_terminal_publish_status": str(
            payload["linux_validation_terminal_publish_status"]
        ),
        "workflow_linux_validation_terminal_publish_decision": str(
            payload["linux_validation_terminal_publish_decision"]
        ),
        "workflow_linux_validation_terminal_publish_exit_code": str(
            payload["linux_validation_terminal_publish_exit_code"]
        ),
        "workflow_linux_validation_terminal_publish_should_notify": (
            "true" if payload["linux_validation_terminal_publish_should_notify"] else "false"
        ),
        "workflow_linux_validation_terminal_publish_ready_for_handoff": (
            "true"
            if payload["linux_validation_terminal_publish_ready_for_handoff"]
            else "false"
        ),
        "workflow_linux_validation_terminal_publish_requires_manual_action": (
            "true"
            if payload["linux_validation_terminal_publish_requires_manual_action"]
            else "false"
        ),
        "workflow_linux_validation_terminal_publish_channel": str(
            payload["linux_validation_terminal_publish_channel"]
        ),
        "workflow_linux_validation_terminal_publish_follow_up_queue_url": str(
            payload["follow_up_queue_url"]
        ),
        "workflow_linux_validation_terminal_publish_run_id": (
            "" if release_run_id is None else str(release_run_id)
        ),
        "workflow_linux_validation_terminal_publish_run_url": str(payload["release_run_url"]),
        "workflow_linux_validation_terminal_publish_reason_codes": json.dumps(
            payload["reason_codes"],
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        "workflow_linux_validation_terminal_publish_report_json": str(output_json),
        "workflow_linux_validation_terminal_publish_report_markdown": str(output_markdown),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Publish Linux CI workflow Linux validation terminal publish contract "
            "from P2-62 Linux validation verdict publish report"
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--print-report",
        action="store_true",
        help="Print Linux validation terminal publish JSON payload",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    verdict_publish_report_path = Path(args.linux_validation_verdict_publish_report)
    output_json_path = Path(args.output_json)
    output_markdown_path = Path(args.output_markdown)

    try:
        verdict_publish_report = load_linux_validation_verdict_publish_report(
            verdict_publish_report_path
        )
        payload = build_linux_validation_terminal_publish_payload(
            verdict_publish_report,
            source_path=verdict_publish_report_path.resolve(),
        )
    except ValueError as exc:
        print(
            "[p2-linux-ci-workflow-linux-validation-terminal-publish-gate] "
            f"{exc}",
            file=sys.stderr,
        )
        return 2
    ...
    if not args.dry_run:
        ...
        print(f"linux validation terminal publish json: {output_json_path}")
        print(f"linux validation terminal publish markdown: {output_markdown_path}")
    ...
    print(
        "linux validation terminal publish summary: "
        f"linux_validation_terminal_publish_status="
        f"{payload['linux_validation_terminal_publish_status']} "
        f"linux_validation_terminal_publish_decision="
        f"{payload['linux_validation_terminal_publish_decision']} "
        f"linux_validation_terminal_publish_exit_code="
        f"{payload['linux_validation_terminal_publish_exit_code']}"
    )
    if args.print_report or args.dry_run:
        print(json.dumps(payload, indent=2, ensure_ascii=False))

    return int(payload["linux_validation_terminal_publish_exit_code"])
