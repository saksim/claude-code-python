"""Phase 2 card P2-62 gate for Linux CI workflow Linux-validation verdict publish.

This script consumes the P2-61 Linux-validation verdict artifact and converges
one terminal publish contract for Linux validation:
1) validate Linux validation verdict + evidence consistency,
...
def load_linux_validation_verdict_report(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError(f"{path}: Linux validation verdict report not found")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"{path}: invalid JSON ({exc})") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"{path}: Linux validation verdict payload must be object")
    ...
    if verdict_status not in ALLOWED_LINUX_VALIDATION_VERDICT_STATUSES:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_status' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_VERDICT_STATUSES)}"
        )
    ...
    if verdict_decision not in ALLOWED_LINUX_VALIDATION_VERDICT_DECISIONS:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_decision' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_VERDICT_DECISIONS)}"
        )
    ...
    if verdict_exit_code < 0:
        raise ValueError(f"{path}: field 'linux_validation_verdict_exit_code' must be >= 0")
    ...
    if verdict_channel not in ALLOWED_LINUX_VALIDATION_VERDICT_CHANNELS:
        raise ValueError(
            f"{path}: field 'linux_validation_verdict_channel' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_VERDICT_CHANNELS)}"
        )
    ...
    return {
        ...
        "linux_validation_verdict_channel": verdict_channel,
...
def build_linux_validation_verdict_publish_payload(
    linux_validation_verdict_report: dict[str, Any], *, source_path: Path
) -> dict[str, Any]:
    verdict_status = str(linux_validation_verdict_report["linux_validation_verdict_status"])
    verdict_decision = str(linux_validation_verdict_report["linux_validation_verdict_decision"])
    verdict_exit_code = int(linux_validation_verdict_report["linux_validation_verdict_exit_code"])
    verdict_passed = bool(linux_validation_verdict_report["linux_validation_passed"])
    verdict_requires_manual_action = bool(
        linux_validation_verdict_report["linux_validation_verdict_requires_manual_action"]
    )
    verdict_channel = str(linux_validation_verdict_report["linux_validation_verdict_channel"])

    reason_codes = list(linux_validation_verdict_report["reason_codes"])
    structural_issues = list(linux_validation_verdict_report["structural_issues"])
    missing_artifacts = list(linux_validation_verdict_report["missing_artifacts"])
    evidence_manifest = list(linux_validation_verdict_report["evidence_manifest"])
    ...
    if missing_evidence:
        ...
        reason_codes.append("linux_validation_verdict_publish_evidence_missing")
    ...
    if verdict_decision != expected_decision:
        structural_issues.append("linux_validation_verdict_decision_mismatch")
    if verdict_passed != expected_passed:
        structural_issues.append("linux_validation_verdict_passed_mismatch")
    if verdict_requires_manual_action != expected_requires_manual_action:
        structural_issues.append("linux_validation_verdict_requires_manual_action_mismatch")
    if verdict_channel != expected_channel:
        structural_issues.append("linux_validation_verdict_channel_mismatch")
    if expected_exit_code == 0 and verdict_exit_code != 0:
        structural_issues.append("linux_validation_verdict_exit_code_mismatch_validated")
    if expected_exit_code is None and verdict_exit_code == 0 and verdict_status != "validated":
        structural_issues.append("linux_validation_verdict_exit_code_mismatch_non_validated")
    ...
    if structural_issues or missing_artifacts:
        ...
        publish_decision = "announce_linux_validation_passed"
        publish_exit_code = 0
        publish_should_notify = True
        ...
        publish_decision = "announce_linux_validation_passed_with_follow_up"
        publish_exit_code = max(1, verdict_exit_code)
        publish_should_notify = True
        publish_requires_manual_action = False
        publish_channel = "follow_up"
        reason_codes.append("linux_validation_verdict_publish_with_follow_up")
    elif verdict_status == "validation_failed":
        publish_status = "blocked"
        publish_decision = "announce_linux_validation_blocker"
        publish_exit_code = max(1, verdict_exit_code)
        publish_should_notify = True
        publish_requires_manual_action = True
        publish_channel = "blocker"
        reason_codes.append("linux_validation_verdict_publish_validation_failed")
        ...
        publish_requires_manual_action = True
        publish_channel = "blocker"
        reason_codes.append("linux_validation_verdict_publish_blocked")
    else:
        publish_status = "contract_failed"
        publish_decision = "abort_publish"
        publish_exit_code = max(1, verdict_exit_code)
        ...
        publish_requires_manual_action = True
        publish_channel = "blocker"
        reason_codes.append("linux_validation_verdict_publish_upstream_contract_failed")

    if publish_status not in ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_STATUSES:
        raise ValueError("internal: unsupported linux_validation_verdict_publish_status computed")
    if publish_decision not in ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_DECISIONS:
        raise ValueError("internal: unsupported linux_validation_verdict_publish_decision computed")
    if publish_channel not in ALLOWED_LINUX_VALIDATION_VERDICT_PUBLISH_CHANNELS:
        raise ValueError("internal: unsupported linux_validation_verdict_publish_channel computed")

    reason_codes = _unique(reason_codes)
    summary = (
        f"linux_validation_verdict_status={verdict_status} "
        f"linux_validation_verdict_publish_status={publish_status} "
        f"linux_validation_verdict_publish_decision={publish_decision}"
    )

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_linux_validation_verdict_report": str(source_path),
        "source_linux_validation_dispatch_report": str(
            linux_validation_verdict_report["source_linux_validation_dispatch_report"]
        ),
        "project_root": str(linux_validation_verdict_report["project_root"]),
        "source_terminal_verdict_report": str(
            linux_validation_verdict_report["source_terminal_verdict_report"]
        ),
        "source_release_final_publish_archive_report": str(
            linux_validation_verdict_report["source_release_final_publish_archive_report"]
        ),
        "source_gate_manifest_drift_report": str(
            linux_validation_verdict_report["source_gate_manifest_drift_report"]
        ),
        "release_final_publish_archive_status": str(
            linux_validation_verdict_report["release_final_publish_archive_status"]
        ),
        "gate_manifest_drift_status": str(
            linux_validation_verdict_report["gate_manifest_drift_status"]
        ),
        "gate_manifest_drift_missing_runtime_tests": list(
            linux_validation_verdict_report["gate_manifest_drift_missing_runtime_tests"]
        ),
        "gate_manifest_drift_missing_manifest_entries": list(
            linux_validation_verdict_report["gate_manifest_drift_missing_manifest_entries"]
        ),
        "gate_manifest_drift_orphan_manifest_entries": list(
            linux_validation_verdict_report["gate_manifest_drift_orphan_manifest_entries"]
        ),
        "terminal_verdict_status": str(linux_validation_verdict_report["terminal_verdict_status"]),
        "terminal_verdict_decision": str(
            linux_validation_verdict_report["terminal_verdict_decision"]
        ),
        "terminal_verdict_exit_code": int(
            linux_validation_verdict_report["terminal_verdict_exit_code"]
        ),
        "terminal_verdict_should_proceed": bool(
            linux_validation_verdict_report["terminal_verdict_should_proceed"]
        ),
        "terminal_verdict_requires_manual_action": bool(
            linux_validation_verdict_report["terminal_verdict_requires_manual_action"]
        ),
        "terminal_verdict_channel": str(linux_validation_verdict_report["terminal_verdict_channel"]),
        "linux_validation_should_dispatch": bool(
            linux_validation_verdict_report["linux_validation_should_dispatch"]
        ),
        "linux_validation_requires_manual_action": bool(
            linux_validation_verdict_report["linux_validation_requires_manual_action"]
        ),
        "linux_validation_channel": str(linux_validation_verdict_report["linux_validation_channel"]),
        "linux_validation_dispatch_status": str(
            linux_validation_verdict_report["linux_validation_dispatch_status"]
        ),
        "linux_validation_dispatch_decision": str(
            linux_validation_verdict_report["linux_validation_dispatch_decision"]
        ),
        "linux_validation_dispatch_exit_code": int(
            linux_validation_verdict_report["linux_validation_dispatch_exit_code"]
        ),
        "linux_validation_dispatch_attempted": bool(
            linux_validation_verdict_report["linux_validation_dispatch_attempted"]
        ),
        "linux_validation_dispatch_command_returncode": linux_validation_verdict_report[
            "linux_validation_dispatch_command_returncode"
        ],
        "linux_validation_verdict_status": verdict_status,
        "linux_validation_verdict_decision": verdict_decision,
        "linux_validation_verdict_exit_code": verdict_exit_code,
        "linux_validation_passed": verdict_passed,
        "linux_validation_verdict_requires_manual_action": verdict_requires_manual_action,
        "linux_validation_verdict_channel": verdict_channel,
        "linux_validation_verdict_attempted": bool(
            linux_validation_verdict_report["linux_validation_verdict_attempted"]
        ),
        "linux_validation_verdict_timeout_seconds": int(
            linux_validation_verdict_report["linux_validation_verdict_timeout_seconds"]
        ),
        "linux_validation_verdict_command": str(
            linux_validation_verdict_report["linux_validation_verdict_command"]
        ),
        "linux_validation_verdict_command_parts": list(
            linux_validation_verdict_report["linux_validation_verdict_command_parts"]
        ),
        "dry_run": bool(linux_validation_verdict_report["dry_run"]),
        "verdict_command_returncode": linux_validation_verdict_report["verdict_command_returncode"],
        "verdict_command_stdout_tail": str(
            linux_validation_verdict_report["verdict_command_stdout_tail"]
        ),
        "verdict_command_stderr_tail": str(
            linux_validation_verdict_report["verdict_command_stderr_tail"]
        ),
        "verdict_error_type": str(linux_validation_verdict_report["verdict_error_type"]),
        "verdict_error_message": str(linux_validation_verdict_report["verdict_error_message"]),
        "release_run_id": linux_validation_verdict_report["release_run_id"],
        "release_run_url": str(linux_validation_verdict_report["release_run_url"]),
        "follow_up_queue_url": str(linux_validation_verdict_report["follow_up_queue_url"]),
        "linux_validation_dispatch_summary": str(
            linux_validation_verdict_report["linux_validation_dispatch_summary"]
        ),
        "linux_validation_verdict_summary": str(
            linux_validation_verdict_report["linux_validation_verdict_summary"]
        ),
        "linux_validation_verdict_publish_status": publish_status,
        "linux_validation_verdict_publish_decision": publish_decision,
        "linux_validation_verdict_publish_exit_code": publish_exit_code,
        "linux_validation_verdict_publish_should_notify": publish_should_notify,
        "linux_validation_verdict_publish_requires_manual_action": publish_requires_manual_action,
        "linux_validation_verdict_publish_channel": publish_channel,
        "linux_validation_verdict_publish_summary": summary,
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    lines = [
        "# Linux CI Workflow Linux Validation Verdict Publish Report",
        "",
        (
            "- Linux Validation Verdict Publish Status: "
            f"**{str(payload['linux_validation_verdict_publish_status']).upper()}**"
        ),
        (
            "- Linux Validation Verdict Publish Decision: "
            f"`{payload['linux_validation_verdict_publish_decision']}`"
        ),
        (
            "- Linux Validation Verdict Publish Exit Code: "
            f"`{payload['linux_validation_verdict_publish_exit_code']}`"
        ),
        (
            "- Linux Validation Verdict Publish Should Notify: "
            f"`{payload['linux_validation_verdict_publish_should_notify']}`"
        ),
        (
            "- Linux Validation Verdict Publish Requires Manual Action: "
            f"`{payload['linux_validation_verdict_publish_requires_manual_action']}`"
        ),
        (
            "- Linux Validation Verdict Publish Channel: "
            f"`{payload['linux_validation_verdict_publish_channel']}`"
        ),
        f"- Linux Validation Verdict Status: `{payload['linux_validation_verdict_status']}`",
        ...
        "## Summary",
        "",
        f"- {payload['linux_validation_verdict_publish_summary']}",
        "",
        "## Reason Codes",
    ]
...
def build_github_output_values(
    ...
    return {
        "workflow_linux_validation_verdict_publish_status": str(
            payload["linux_validation_verdict_publish_status"]
        ),
        "workflow_linux_validation_verdict_publish_decision": str(
            payload["linux_validation_verdict_publish_decision"]
        ),
        "workflow_linux_validation_verdict_publish_exit_code": str(
            payload["linux_validation_verdict_publish_exit_code"]
        ),
        "workflow_linux_validation_verdict_publish_should_notify": (
            "true" if payload["linux_validation_verdict_publish_should_notify"] else "false"
        ),
        "workflow_linux_validation_verdict_publish_requires_manual_action": (
            "true"
            if payload["linux_validation_verdict_publish_requires_manual_action"]
            else "false"
        ),
        "workflow_linux_validation_verdict_publish_channel": str(
            payload["linux_validation_verdict_publish_channel"]
        ),
        "workflow_linux_validation_verdict_publish_follow_up_queue_url": str(
            payload["follow_up_queue_url"]
        ),
        "workflow_linux_validation_verdict_publish_run_id": (
            "" if release_run_id is None else str(release_run_id)
        ),
        "workflow_linux_validation_verdict_publish_run_url": str(payload["release_run_url"]),
        "workflow_linux_validation_verdict_publish_reason_codes": json.dumps(
            payload["reason_codes"],
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        "workflow_linux_validation_verdict_publish_report_json": str(output_json),
        "workflow_linux_validation_verdict_publish_report_markdown": str(output_markdown),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--print-report",
        action="store_true",
        help="Print Linux validation verdict publish JSON payload",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    verdict_report_path = Path(args.linux_validation_verdict_report)
    output_json_path = Path(args.output_json)
    output_markdown_path = Path(args.output_markdown)

    try:
        verdict_report = load_linux_validation_verdict_report(verdict_report_path)
        payload = build_linux_validation_verdict_publish_payload(
            verdict_report,
            source_path=verdict_report_path.resolve(),
        )
    except ValueError as exc:
        print(
            "[p2-linux-ci-workflow-linux-validation-verdict-publish-gate] "
            f"{exc}",
            file=sys.stderr,
        )
        return 2
    ...
    if not args.dry_run:
        ...
        print(f"linux validation verdict publish json: {output_json_path}")
        print(f"linux validation verdict publish markdown: {output_markdown_path}")
    ...
    print(
        "linux validation verdict publish summary: "
        f"linux_validation_verdict_publish_status="
        f"{payload['linux_validation_verdict_publish_status']} "
        f"linux_validation_verdict_publish_decision="
        f"{payload['linux_validation_verdict_publish_decision']} "
        f"linux_validation_verdict_publish_exit_code="
        f"{payload['linux_validation_verdict_publish_exit_code']}"
    )
    if args.print_report or args.dry_run:
        print(json.dumps(payload, indent=2, ensure_ascii=False))

    return int(payload["linux_validation_verdict_publish_exit_code"])
