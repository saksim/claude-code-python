"""Phase 2 card P2-50 gate for Linux CI workflow release final handoff.

This script consumes the P2-49 release final terminal publish artifact and
converges one terminal handoff contract:
1) validate release final terminal publish + evidence consistency,
2) normalize release handoff semantics for downstream consumers,
...
def load_release_final_terminal_publish_report(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError(f"{path}: release final terminal publish report not found")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"{path}: invalid JSON ({exc})") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"{path}: release final terminal publish payload must be object")
    ...
    required_fields = (
        ...
        "final_terminal_should_notify",
        "final_terminal_requires_manual_action",
        "final_terminal_channel",
    ...
    final_terminal_channel = _coerce_str(
        payload["final_terminal_channel"],
        field="final_terminal_channel",
        path=path,
    )
    if final_terminal_channel not in ALLOWED_RELEASE_FINAL_TERMINAL_PUBLISH_CHANNELS:
        raise ValueError(
            f"{path}: field 'final_terminal_channel' must be one of "
            f"{sorted(ALLOWED_RELEASE_FINAL_TERMINAL_PUBLISH_CHANNELS)}"
        )
    ...
    return {
        ...
        "final_terminal_should_notify": _coerce_bool(
            payload["final_terminal_should_notify"],
            field="final_terminal_should_notify",
            path=path,
        ),
        "final_terminal_requires_manual_action": _coerce_bool(
            payload["final_terminal_requires_manual_action"],
            field="final_terminal_requires_manual_action",
            path=path,
        ),
        "final_terminal_channel": final_terminal_channel,
        "release_final_terminal_publish_summary": _coerce_str(
            payload["release_final_terminal_publish_summary"],
            field="release_final_terminal_publish_summary",
            path=path,
        ),
...
def build_release_final_handoff_payload(
    release_final_terminal_publish_report: dict[str, Any], *, source_path: Path
) -> dict[str, Any]:
    final_terminal_status = str(
        release_final_terminal_publish_report["release_final_terminal_publish_status"]
    )
    final_terminal_decision = str(
        release_final_terminal_publish_report["release_final_terminal_publish_decision"]
    )
    final_terminal_exit_code = int(
        release_final_terminal_publish_report["release_final_terminal_publish_exit_code"]
    )
    final_terminal_should_notify = bool(
        release_final_terminal_publish_report["final_terminal_should_notify"]
    )
    final_terminal_requires_manual_action = bool(
        release_final_terminal_publish_report["final_terminal_requires_manual_action"]
    )
    final_terminal_channel = str(release_final_terminal_publish_report["final_terminal_channel"])

    final_should_ship_release = bool(
        release_final_terminal_publish_report["final_should_ship_release"]
    )
    final_follow_up_open = bool(release_final_terminal_publish_report["final_follow_up_open"])
    final_should_page_owner = bool(
        release_final_terminal_publish_report["final_should_page_owner"]
    )
    final_outcome_target = str(release_final_terminal_publish_report["final_outcome_target"])

    reason_codes = list(release_final_terminal_publish_report["reason_codes"])
    structural_issues = list(release_final_terminal_publish_report["structural_issues"])
    missing_artifacts = list(release_final_terminal_publish_report["missing_artifacts"])
    evidence_manifest = list(release_final_terminal_publish_report["evidence_manifest"])
    ...
    if missing_evidence:
        ...
        reason_codes.append("release_final_handoff_evidence_missing")
    ...
    (
        ...
        expected_page,
        expected_target,
    ) = expected[final_terminal_status]

    if final_terminal_decision != expected_decision:
        structural_issues.append("final_terminal_decision_mismatch")
    if final_terminal_should_notify != expected_notify:
        structural_issues.append("final_terminal_should_notify_mismatch")
    if final_terminal_requires_manual_action != expected_manual:
        structural_issues.append("final_terminal_requires_manual_action_mismatch")
    if final_terminal_channel != expected_channel:
        structural_issues.append("final_terminal_channel_mismatch")
    ...
    if expected_exit_code == 0 and final_terminal_exit_code != 0:
        structural_issues.append("final_terminal_exit_code_mismatch_published")
    if expected_exit_code is None and final_terminal_exit_code == 0:
        structural_issues.append("final_terminal_exit_code_mismatch_non_published")
    ...
    if structural_issues or missing_artifacts:
        ...
        handoff_decision = "abort_handoff"
        handoff_exit_code = max(1, final_terminal_exit_code)
        final_handoff_should_archive_release = False
        final_handoff_keep_follow_up_open = True
        final_handoff_should_page_owner = True
        final_handoff_target = "blocker"
        reason_codes.extend(structural_issues)
    elif final_terminal_status == "published":
        handoff_status = "completed"
        handoff_decision = "handoff_release_closure"
        handoff_exit_code = 0
    ...
    elif final_terminal_status == "published_with_follow_up":
        handoff_status = "completed_with_follow_up"
        handoff_decision = "handoff_release_with_follow_up"
        handoff_exit_code = max(1, final_terminal_exit_code)
        final_handoff_should_archive_release = True
        final_handoff_keep_follow_up_open = True
        final_handoff_should_page_owner = False
        final_handoff_target = "follow_up"
        reason_codes.append("release_final_handoff_completed_with_follow_up")
    elif final_terminal_status == "blocked":
        handoff_status = "blocked"
        handoff_decision = "handoff_blocker"
        handoff_exit_code = max(1, final_terminal_exit_code)
        final_handoff_should_archive_release = False
        final_handoff_keep_follow_up_open = True
        final_handoff_should_page_owner = True
        final_handoff_target = "blocker"
        reason_codes.append("release_final_handoff_blocked")
    else:
        handoff_status = "contract_failed"
        handoff_decision = "abort_handoff"
        handoff_exit_code = max(1, final_terminal_exit_code)
        ...
        final_handoff_should_page_owner = True
        final_handoff_target = "blocker"
        reason_codes.append("release_final_handoff_upstream_contract_failed")

    if handoff_status not in ALLOWED_RELEASE_FINAL_HANDOFF_STATUSES:
        raise ValueError("internal: unsupported release_final_handoff_status computed")
    if handoff_decision not in ALLOWED_RELEASE_FINAL_HANDOFF_DECISIONS:
        raise ValueError("internal: unsupported release_final_handoff_decision computed")
    if final_handoff_target not in ALLOWED_RELEASE_FINAL_HANDOFF_TARGETS:
        raise ValueError("internal: unsupported release_final_handoff_target computed")

    reason_codes = _unique(reason_codes)
    summary = (
        f"release_final_terminal_publish_status={final_terminal_status} "
        f"release_final_handoff_status={handoff_status} "
        f"release_final_handoff_decision={handoff_decision}"
    )

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_release_final_terminal_publish_report": str(source_path),
        "source_release_final_outcome_report": str(
            release_final_terminal_publish_report["source_release_final_outcome_report"]
        ),
        "source_release_delivery_final_verdict_report": str(
            release_final_terminal_publish_report["source_release_delivery_final_verdict_report"]
        ),
        "source_release_follow_up_final_verdict_report": str(
            release_final_terminal_publish_report["source_release_follow_up_final_verdict_report"]
        ),
        "release_delivery_final_verdict_status": str(
            release_final_terminal_publish_report["release_delivery_final_verdict_status"]
        ),
        "release_delivery_final_verdict_decision": str(
            release_final_terminal_publish_report["release_delivery_final_verdict_decision"]
        ),
        "release_delivery_final_verdict_exit_code": int(
            release_final_terminal_publish_report["release_delivery_final_verdict_exit_code"]
        ),
        "release_follow_up_final_verdict_status": str(
            release_final_terminal_publish_report["release_follow_up_final_verdict_status"]
        ),
        "release_follow_up_final_verdict_decision": str(
            release_final_terminal_publish_report["release_follow_up_final_verdict_decision"]
        ),
        "release_follow_up_final_verdict_exit_code": int(
            release_final_terminal_publish_report["release_follow_up_final_verdict_exit_code"]
        ),
        "release_final_outcome_status": str(
            release_final_terminal_publish_report["release_final_outcome_status"]
        ),
        "release_final_outcome_decision": str(
            release_final_terminal_publish_report["release_final_outcome_decision"]
        ),
        "release_final_outcome_exit_code": int(
            release_final_terminal_publish_report["release_final_outcome_exit_code"]
        ),
        "release_final_terminal_publish_status": final_terminal_status,
        "release_final_terminal_publish_decision": final_terminal_decision,
        "release_final_terminal_publish_exit_code": final_terminal_exit_code,
        "release_run_id": release_final_terminal_publish_report["release_run_id"],
        "release_run_url": str(release_final_terminal_publish_report["release_run_url"]),
        "follow_up_queue_url": str(release_final_terminal_publish_report["follow_up_queue_url"]),
        "final_handoff_should_archive_release": final_handoff_should_archive_release,
        "final_handoff_keep_follow_up_open": final_handoff_keep_follow_up_open,
        "final_handoff_should_page_owner": final_handoff_should_page_owner,
        "final_handoff_target": final_handoff_target,
        "release_final_handoff_status": handoff_status,
        "release_final_handoff_decision": handoff_decision,
        "release_final_handoff_exit_code": handoff_exit_code,
        "release_final_handoff_summary": summary,
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    lines = [
        "# Linux CI Workflow Release Final Handoff Report",
        "",
        f"- Release Final Handoff Status: **{str(payload['release_final_handoff_status']).upper()}**",
        f"- Release Final Handoff Decision: `{payload['release_final_handoff_decision']}`",
        f"- Release Final Handoff Exit Code: `{payload['release_final_handoff_exit_code']}`",
        (
            f"- Final Handoff Should Archive Release: "
            f"`{payload['final_handoff_should_archive_release']}`"
        ),
        (
            f"- Final Handoff Keep Follow-Up Open: "
            f"`{payload['final_handoff_keep_follow_up_open']}`"
        ),
        (
            f"- Final Handoff Should Page Owner: "
            f"`{payload['final_handoff_should_page_owner']}`"
        ),
        f"- Final Handoff Target: `{payload['final_handoff_target']}`",
        (
            f"- Release Final Terminal Publish Status: "
            f"`{payload['release_final_terminal_publish_status']}`"
        ),
        ...
        "## Summary",
        "",
        f"- {payload['release_final_handoff_summary']}",
        "",
        "## Reason Codes",
    ]
...
def build_github_output_values(
    ...
    return {
        "workflow_release_final_handoff_status": str(payload["release_final_handoff_status"]),
        "workflow_release_final_handoff_decision": str(payload["release_final_handoff_decision"]),
        "workflow_release_final_handoff_exit_code": str(payload["release_final_handoff_exit_code"]),
        "workflow_release_final_handoff_should_archive_release": (
            "true" if payload["final_handoff_should_archive_release"] else "false"
        ),
        "workflow_release_final_handoff_keep_follow_up_open": (
            "true" if payload["final_handoff_keep_follow_up_open"] else "false"
        ),
        "workflow_release_final_handoff_should_page_owner": (
            "true" if payload["final_handoff_should_page_owner"] else "false"
        ),
        "workflow_release_final_handoff_target": str(payload["final_handoff_target"]),
        "workflow_release_final_handoff_follow_up_queue_url": str(payload["follow_up_queue_url"]),
        "workflow_release_final_handoff_run_id": (
            "" if release_run_id is None else str(release_run_id)
        ),
        "workflow_release_final_handoff_run_url": str(payload["release_run_url"]),
        "workflow_release_final_handoff_reason_codes": json.dumps(
            payload["reason_codes"],
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        "workflow_release_final_handoff_report_json": str(output_json),
        "workflow_release_final_handoff_report_markdown": str(output_markdown),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Publish Linux CI workflow release final handoff contract "
            "from P2-49 release final terminal publish report"
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--print-report",
        action="store_true",
        help="Print release final handoff JSON payload",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    final_terminal_publish_report_path = Path(args.release_final_terminal_publish_report)
    output_json_path = Path(args.output_json)
    output_markdown_path = Path(args.output_markdown)

    try:
        final_terminal_publish_report = load_release_final_terminal_publish_report(
            final_terminal_publish_report_path
        )
        payload = build_release_final_handoff_payload(
            final_terminal_publish_report,
            source_path=final_terminal_publish_report_path.resolve(),
        )
    except ValueError as exc:
        print(f"[p2-linux-ci-workflow-release-final-handoff-gate] {exc}", file=sys.stderr)
        return 2
    ...
    if not args.dry_run:
        ...
        print(f"release final handoff json: {output_json_path}")
        print(f"release final handoff markdown: {output_markdown_path}")
    ...
    print(
        "release final handoff summary: "
        f"release_final_handoff_status={payload['release_final_handoff_status']} "
        f"release_final_handoff_decision={payload['release_final_handoff_decision']} "
        f"release_final_handoff_exit_code={payload['release_final_handoff_exit_code']}"
    )
    if args.print_report or args.dry_run:
        print(json.dumps(payload, indent=2, ensure_ascii=False))

    return int(payload["release_final_handoff_exit_code"])
