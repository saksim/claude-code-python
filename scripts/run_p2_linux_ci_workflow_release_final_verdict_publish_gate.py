"""Phase 2 card P2-55 gate for Linux CI workflow release final verdict publish.

This script consumes the P2-54 release final verdict artifact and converges
one terminal final-verdict-publish contract:
1) validate release final verdict + evidence consistency,
2) normalize final publish semantics for downstream consumers,
...
def load_release_final_verdict_report(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError(f"{path}: release final verdict report not found")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"{path}: invalid JSON ({exc})") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"{path}: release final verdict payload must be object")
    ...
    required_fields = (
        ...
        "release_run_url",
        "follow_up_queue_url",
        "final_verdict_should_ship",
        "final_verdict_requires_manual_action",
        "final_verdict_channel",
    ...
    if release_final_verdict_status not in ALLOWED_RELEASE_FINAL_VERDICT_STATUSES:
        raise ValueError(
            f"{path}: field 'release_final_verdict_status' must be one of "
            f"{sorted(ALLOWED_RELEASE_FINAL_VERDICT_STATUSES)}"
        )
    ...
    if release_final_verdict_decision not in ALLOWED_RELEASE_FINAL_VERDICT_DECISIONS:
        raise ValueError(
            f"{path}: field 'release_final_verdict_decision' must be one of "
            f"{sorted(ALLOWED_RELEASE_FINAL_VERDICT_DECISIONS)}"
        )
    ...
    if release_final_verdict_exit_code < 0:
        raise ValueError(f"{path}: field 'release_final_verdict_exit_code' must be >= 0")
    final_verdict_channel = _coerce_str(
        payload["final_verdict_channel"],
        field="final_verdict_channel",
        path=path,
    )
    if final_verdict_channel not in ALLOWED_RELEASE_FINAL_VERDICT_CHANNELS:
        raise ValueError(
            f"{path}: field 'final_verdict_channel' must be one of "
            f"{sorted(ALLOWED_RELEASE_FINAL_VERDICT_CHANNELS)}"
        )
    ...
    return {
        ...
        "final_verdict_should_ship": _coerce_bool(
            payload["final_verdict_should_ship"],
            field="final_verdict_should_ship",
            path=path,
        ),
        "final_verdict_requires_manual_action": _coerce_bool(
            payload["final_verdict_requires_manual_action"],
            field="final_verdict_requires_manual_action",
            path=path,
        ),
        "final_verdict_channel": final_verdict_channel,
        "release_final_verdict_status": release_final_verdict_status,
        "release_final_verdict_decision": release_final_verdict_decision,
        "release_final_verdict_exit_code": release_final_verdict_exit_code,
        "release_final_verdict_summary": _coerce_str(
            payload["release_final_verdict_summary"],
            field="release_final_verdict_summary",
            path=path,
        ),
...
def build_release_final_verdict_publish_payload(
    release_final_verdict_report: dict[str, Any], *, source_path: Path
) -> dict[str, Any]:
    final_verdict_status = str(release_final_verdict_report["release_final_verdict_status"])
    final_verdict_decision = str(release_final_verdict_report["release_final_verdict_decision"])
    final_verdict_exit_code = int(release_final_verdict_report["release_final_verdict_exit_code"])
    final_verdict_should_ship = bool(release_final_verdict_report["final_verdict_should_ship"])
    final_verdict_requires_manual_action = bool(
        release_final_verdict_report["final_verdict_requires_manual_action"]
    )
    final_verdict_channel = str(release_final_verdict_report["final_verdict_channel"])

    reason_codes = list(release_final_verdict_report["reason_codes"])
    structural_issues = list(release_final_verdict_report["structural_issues"])
    missing_artifacts = list(release_final_verdict_report["missing_artifacts"])
    evidence_manifest = list(release_final_verdict_report["evidence_manifest"])
    ...
    if missing_evidence:
        ...
        reason_codes.append("release_final_verdict_publish_evidence_missing")
    ...
    (
        ...
        expected_channel,
    ) = expected[final_verdict_status]

    if final_verdict_decision != expected_decision:
        structural_issues.append("final_verdict_decision_mismatch")
    if final_verdict_should_ship != expected_should_ship:
        structural_issues.append("final_verdict_should_ship_mismatch")
    if final_verdict_requires_manual_action != expected_requires_manual_action:
        structural_issues.append("final_verdict_requires_manual_action_mismatch")
    if final_verdict_channel != expected_channel:
        structural_issues.append("final_verdict_channel_mismatch")
    if expected_exit_code == 0 and final_verdict_exit_code != 0:
        structural_issues.append("final_verdict_exit_code_mismatch_released")
    if expected_exit_code is None and final_verdict_exit_code == 0:
        structural_issues.append("final_verdict_exit_code_mismatch_non_released")
    ...
    if structural_issues or missing_artifacts:
    ...
    elif final_verdict_status == "released":
        publish_status = "published"
        publish_decision = "announce_release_shipped"
        publish_exit_code = 0
        publish_should_notify = True
        publish_requires_manual_action = False
        publish_channel = "release"
        reason_codes = ["release_final_verdict_publish_published"]
    elif final_verdict_status == "released_with_follow_up":
        publish_status = "published_with_follow_up"
        publish_decision = "announce_release_shipped_with_follow_up"
        publish_exit_code = max(1, final_verdict_exit_code)
        publish_should_notify = True
        publish_requires_manual_action = False
        publish_channel = "follow_up"
        reason_codes.append("release_final_verdict_publish_with_follow_up")
        ...
        publish_requires_manual_action = True
        publish_channel = "blocker"
        reason_codes.append("release_final_verdict_publish_blocked")
    else:
        publish_status = "contract_failed"
        publish_decision = "abort_publish"
        publish_exit_code = max(1, final_verdict_exit_code)
        ...
        publish_requires_manual_action = True
        publish_channel = "blocker"
        reason_codes.append("release_final_verdict_publish_upstream_contract_failed")

    if publish_status not in ALLOWED_RELEASE_FINAL_VERDICT_PUBLISH_STATUSES:
        raise ValueError("internal: unsupported release_final_verdict_publish_status computed")
    if publish_decision not in ALLOWED_RELEASE_FINAL_VERDICT_PUBLISH_DECISIONS:
        raise ValueError("internal: unsupported release_final_verdict_publish_decision computed")
    if publish_channel not in ALLOWED_RELEASE_FINAL_VERDICT_PUBLISH_CHANNELS:
        raise ValueError("internal: unsupported release_final_verdict_publish_channel computed")

    reason_codes = _unique(reason_codes)
    summary = (
        f"release_final_verdict_status={final_verdict_status} "
        f"release_final_verdict_publish_status={publish_status} "
        f"release_final_verdict_publish_decision={publish_decision}"
    )

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_release_final_verdict_report": str(source_path),
        "source_release_final_archive_report": str(
            release_final_verdict_report["source_release_final_archive_report"]
        ),
        "source_release_final_closure_publish_report": str(
            release_final_verdict_report["source_release_final_closure_publish_report"]
        ),
        "source_release_final_closure_report": str(
            release_final_verdict_report["source_release_final_closure_report"]
        ),
        "source_release_final_handoff_report": str(
            release_final_verdict_report["source_release_final_handoff_report"]
        ),
        "source_release_final_terminal_publish_report": str(
            release_final_verdict_report["source_release_final_terminal_publish_report"]
        ),
        "source_release_final_outcome_report": str(
            release_final_verdict_report["source_release_final_outcome_report"]
        ),
        "source_release_delivery_final_verdict_report": str(
            release_final_verdict_report["source_release_delivery_final_verdict_report"]
        ),
        "source_release_follow_up_final_verdict_report": str(
            release_final_verdict_report["source_release_follow_up_final_verdict_report"]
        ),
        "release_delivery_final_verdict_status": str(
            release_final_verdict_report["release_delivery_final_verdict_status"]
        ),
        "release_delivery_final_verdict_decision": str(
            release_final_verdict_report["release_delivery_final_verdict_decision"]
        ),
        "release_delivery_final_verdict_exit_code": int(
            release_final_verdict_report["release_delivery_final_verdict_exit_code"]
        ),
        "release_follow_up_final_verdict_status": str(
            release_final_verdict_report["release_follow_up_final_verdict_status"]
        ),
        "release_follow_up_final_verdict_decision": str(
            release_final_verdict_report["release_follow_up_final_verdict_decision"]
        ),
        "release_follow_up_final_verdict_exit_code": int(
            release_final_verdict_report["release_follow_up_final_verdict_exit_code"]
        ),
        "release_final_outcome_status": str(
            release_final_verdict_report["release_final_outcome_status"]
        ),
        "release_final_outcome_decision": str(
            release_final_verdict_report["release_final_outcome_decision"]
        ),
        "release_final_outcome_exit_code": int(
            release_final_verdict_report["release_final_outcome_exit_code"]
        ),
        "release_final_terminal_publish_status": str(
            release_final_verdict_report["release_final_terminal_publish_status"]
        ),
        "release_final_terminal_publish_decision": str(
            release_final_verdict_report["release_final_terminal_publish_decision"]
        ),
        "release_final_terminal_publish_exit_code": int(
            release_final_verdict_report["release_final_terminal_publish_exit_code"]
        ),
        "release_final_handoff_status": str(release_final_verdict_report["release_final_handoff_status"]),
        "release_final_handoff_decision": str(
            release_final_verdict_report["release_final_handoff_decision"]
        ),
        "release_final_handoff_exit_code": int(
            release_final_verdict_report["release_final_handoff_exit_code"]
        ),
        "release_final_closure_status": str(release_final_verdict_report["release_final_closure_status"]),
        "release_final_closure_decision": str(
            release_final_verdict_report["release_final_closure_decision"]
        ),
        "release_final_closure_exit_code": int(
            release_final_verdict_report["release_final_closure_exit_code"]
        ),
        "release_final_closure_publish_status": str(
            release_final_verdict_report["release_final_closure_publish_status"]
        ),
        "release_final_closure_publish_decision": str(
            release_final_verdict_report["release_final_closure_publish_decision"]
        ),
        "release_final_closure_publish_exit_code": int(
            release_final_verdict_report["release_final_closure_publish_exit_code"]
        ),
        "release_final_archive_status": str(release_final_verdict_report["release_final_archive_status"]),
        "release_final_archive_decision": str(release_final_verdict_report["release_final_archive_decision"]),
        "release_final_archive_exit_code": int(release_final_verdict_report["release_final_archive_exit_code"]),
        ...
        "release_run_id": release_final_verdict_report["release_run_id"],
        "release_run_url": str(release_final_verdict_report["release_run_url"]),
        "follow_up_queue_url": str(release_final_verdict_report["follow_up_queue_url"]),
        "final_verdict_publish_should_notify": publish_should_notify,
        "final_verdict_publish_requires_manual_action": publish_requires_manual_action,
        "final_verdict_publish_channel": publish_channel,
        "release_final_verdict_publish_status": publish_status,
        "release_final_verdict_publish_decision": publish_decision,
        "release_final_verdict_publish_exit_code": publish_exit_code,
        "release_final_verdict_publish_summary": summary,
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    lines = [
        "# Linux CI Workflow Release Final Verdict Publish Report",
        "",
        (
            "- Release Final Verdict Publish Status: "
            f"**{str(payload['release_final_verdict_publish_status']).upper()}**"
        ),
        (
            "- Release Final Verdict Publish Decision: "
            f"`{payload['release_final_verdict_publish_decision']}`"
        ),
        (
            "- Release Final Verdict Publish Exit Code: "
            f"`{payload['release_final_verdict_publish_exit_code']}`"
        ),
        (
            "- Final Verdict Publish Should Notify: "
            f"`{payload['final_verdict_publish_should_notify']}`"
        ),
        (
            "- Final Verdict Publish Requires Manual Action: "
            f"`{payload['final_verdict_publish_requires_manual_action']}`"
        ),
        f"- Final Verdict Publish Channel: `{payload['final_verdict_publish_channel']}`",
        f"- Release Final Verdict Status: `{payload['release_final_verdict_status']}`",
        ...
        "## Summary",
        "",
        f"- {payload['release_final_verdict_publish_summary']}",
        "",
        "## Reason Codes",
    ]
...
def build_github_output_values(
    ...
    return {
        "workflow_release_final_verdict_publish_status": str(
            payload["release_final_verdict_publish_status"]
        ),
        "workflow_release_final_verdict_publish_decision": str(
            payload["release_final_verdict_publish_decision"]
        ),
        "workflow_release_final_verdict_publish_exit_code": str(
            payload["release_final_verdict_publish_exit_code"]
        ),
        "workflow_release_final_verdict_publish_should_notify": (
            "true" if payload["final_verdict_publish_should_notify"] else "false"
        ),
        "workflow_release_final_verdict_publish_requires_manual_action": (
            "true" if payload["final_verdict_publish_requires_manual_action"] else "false"
        ),
        "workflow_release_final_verdict_publish_channel": str(
            payload["final_verdict_publish_channel"]
        ),
        "workflow_release_final_verdict_publish_follow_up_queue_url": str(
            payload["follow_up_queue_url"]
        ),
        "workflow_release_final_verdict_publish_run_id": (
            "" if release_run_id is None else str(release_run_id)
        ),
        "workflow_release_final_verdict_publish_run_url": str(payload["release_run_url"]),
        "workflow_release_final_verdict_publish_reason_codes": json.dumps(
            payload["reason_codes"],
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        "workflow_release_final_verdict_publish_report_json": str(output_json),
        "workflow_release_final_verdict_publish_report_markdown": str(output_markdown),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--print-report",
        action="store_true",
        help="Print release final verdict publish JSON payload",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    release_final_verdict_report_path = Path(args.release_final_verdict_report)
    output_json_path = Path(args.output_json)
    output_markdown_path = Path(args.output_markdown)

    try:
        release_final_verdict_report = load_release_final_verdict_report(
            release_final_verdict_report_path
        )
        payload = build_release_final_verdict_publish_payload(
            release_final_verdict_report,
            source_path=release_final_verdict_report_path.resolve(),
        )
    except ValueError as exc:
        print(
            "[p2-linux-ci-workflow-release-final-verdict-publish-gate] "
            f"{exc}",
            file=sys.stderr,
        )
        return 2
    ...
    if not args.dry_run:
        ...
        print(f"release final verdict publish json: {output_json_path}")
        print(f"release final verdict publish markdown: {output_markdown_path}")
    ...
    print(
        "release final verdict publish summary: "
        f"release_final_verdict_publish_status={payload['release_final_verdict_publish_status']} "
        f"release_final_verdict_publish_decision={payload['release_final_verdict_publish_decision']} "
        f"release_final_verdict_publish_exit_code={payload['release_final_verdict_publish_exit_code']}"
    )
    if args.print_report or args.dry_run:
        print(json.dumps(payload, indent=2, ensure_ascii=False))

    return int(payload["release_final_verdict_publish_exit_code"])
