"""Phase 2 card P2-79 gate for Linux CI workflow Linux validation terminal dispatch terminal verdict publish archive.

This script consumes the P2-78 Linux validation terminal dispatch terminal verdict publish artifact and
converges one terminal verdict publish archive contract:
1) validate Linux validation terminal dispatch terminal verdict publish contract consistency,
2) normalize terminal verdict publish archive semantics for downstream consumers,
...
ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_ARCHIVE_STATUSES: set[str] = {
    "archived",
...
ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_ARCHIVE_DECISIONS: set[str] = {
...
ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_ARCHIVE_CHANNELS: set[str] = {
    "release",
    "follow_up",
    "blocker",
}
...
def load_linux_validation_terminal_dispatch_terminal_verdict_publish_report(
    path: Path,
) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError(
            f"{path}: Linux validation terminal dispatch terminal verdict publish report not found"
        )
    try:
    ...
    if not isinstance(payload, dict):
        raise ValueError(
            f"{path}: Linux validation terminal dispatch terminal verdict publish payload must be object"
        )

    required_fields = (
        "source_linux_validation_terminal_dispatch_terminal_verdict_report",
    ...
    publish_status = _coerce_str(
        ...
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_status",
        path=path,
    )
    if (
        publish_status
        ...
        raise ValueError(
            f"{path}: field 'linux_validation_terminal_dispatch_terminal_verdict_publish_status' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_STATUSES)}"
        )

    publish_decision = _coerce_str(
        payload["linux_validation_terminal_dispatch_terminal_verdict_publish_decision"],
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_decision",
        path=path,
    )
    if (
        publish_decision
        ...
        raise ValueError(
            f"{path}: field 'linux_validation_terminal_dispatch_terminal_verdict_publish_decision' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_DECISIONS)}"
        )

    publish_exit_code = _coerce_int(
        payload["linux_validation_terminal_dispatch_terminal_verdict_publish_exit_code"],
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_exit_code",
        path=path,
    )
    if publish_exit_code < 0:
        raise ValueError(
            f"{path}: field 'linux_validation_terminal_dispatch_terminal_verdict_publish_exit_code' must be >= 0"
        )

    publish_channel = _coerce_str(
        payload["linux_validation_terminal_dispatch_terminal_verdict_publish_channel"],
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_channel",
        path=path,
    )
    if (
        publish_channel
        ...
        raise ValueError(
            f"{path}: field 'linux_validation_terminal_dispatch_terminal_verdict_publish_channel' must be one of "
            f"{sorted(ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_CHANNELS)}"
        )
    ...
    report = dict(payload)
    report["linux_validation_terminal_dispatch_terminal_verdict_publish_status"] = (
        publish_status
    )
    report["linux_validation_terminal_dispatch_terminal_verdict_publish_decision"] = (
        publish_decision
    )
    report["linux_validation_terminal_dispatch_terminal_verdict_publish_exit_code"] = (
        publish_exit_code
    )
    report[
        "linux_validation_terminal_dispatch_terminal_verdict_publish_should_notify"
    ] = _coerce_bool(
        payload["linux_validation_terminal_dispatch_terminal_verdict_publish_should_notify"],
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_should_notify",
        path=path,
    )
    report[
        "linux_validation_terminal_dispatch_terminal_verdict_publish_requires_manual_action"
        ...
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_requires_manual_action",
        path=path,
    )
    report[
        "linux_validation_terminal_dispatch_terminal_verdict_publish_should_page_owner"
    ] = _coerce_bool(
        payload["linux_validation_terminal_dispatch_terminal_verdict_publish_should_page_owner"],
        field="linux_validation_terminal_dispatch_terminal_verdict_publish_should_page_owner",
        path=path,
    )
    report["linux_validation_terminal_dispatch_terminal_verdict_publish_channel"] = (
        publish_channel
    )
...
def build_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_payload(
    terminal_verdict_publish_report: dict[str, Any], *, source_path: Path
) -> dict[str, Any]:
    publish_status = str(
        terminal_verdict_publish_report[
            "linux_validation_terminal_dispatch_terminal_verdict_publish_status"
        ]
    )
    ...
    reason_codes = list(terminal_verdict_publish_report["reason_codes"])
    structural_issues = list(terminal_verdict_publish_report["structural_issues"])
    missing_artifacts = list(terminal_verdict_publish_report["missing_artifacts"])
    ...
    if publish_should_page_owner != expected_should_page_owner:
        structural_issues.append(
            "linux_validation_terminal_dispatch_terminal_verdict_publish_should_page_owner_mismatch"
        )
    if publish_status == "in_progress":
        if publish_channel not in {"release", "follow_up"}:
            structural_issues.append(
                "linux_validation_terminal_dispatch_terminal_verdict_publish_channel_mismatch"
            )
    elif expected_channel is not None and publish_channel != expected_channel:
        structural_issues.append(
            "linux_validation_terminal_dispatch_terminal_verdict_publish_channel_mismatch"
        )
    if expected_exit_code == 0 and publish_exit_code != 0:
        structural_issues.append(
            "linux_validation_terminal_dispatch_terminal_verdict_publish_exit_code_mismatch_zero"
        )
    if (
        expected_exit_code is None
        and publish_exit_code == 0
        and publish_status
        in {"published_with_follow_up", "blocked", "failed", "contract_failed"}
    ):
        structural_issues.append(
            "linux_validation_terminal_dispatch_terminal_verdict_publish_exit_code_mismatch_nonzero"
        )
    ...
    if structural_issues or missing_artifacts:
        ...
        archive_exit_code = max(1, publish_exit_code)
        ...
        archive_decision = "archive_linux_validation_terminal_dispatch_ready"
        archive_exit_code = 0
        ...
        archive_requires_manual_action = False
        archive_should_page_owner = False
        archive_channel = "follow_up" if publish_channel == "follow_up" else "release"
        reason_codes.append(
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_in_progress"
        )
        ...
        archive_channel = "blocker"
        reason_codes.append(
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_upstream_contract_failed"
        )

    if (
        archive_status
        not in ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_ARCHIVE_STATUSES
    ):
        raise ValueError(
            "internal: unsupported linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status computed"
        )
    if (
        archive_decision
        not in ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_ARCHIVE_DECISIONS
    ):
        raise ValueError(
            "internal: unsupported linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision computed"
        )
    if (
        archive_channel
        not in ALLOWED_LINUX_VALIDATION_TERMINAL_DISPATCH_TERMINAL_VERDICT_PUBLISH_ARCHIVE_CHANNELS
    ):
        raise ValueError(
            "internal: unsupported linux_validation_terminal_dispatch_terminal_verdict_publish_archive_channel computed"
        )
    ...
    payload = dict(terminal_verdict_publish_report)
    payload.update(
        {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "source_linux_validation_terminal_dispatch_terminal_verdict_publish_report": str(
                source_path
            ),
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status": archive_status,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision": archive_decision,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_exit_code": archive_exit_code,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_should_archive": archive_should_archive,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_requires_manual_action": archive_requires_manual_action,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_should_page_owner": archive_should_page_owner,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_channel": archive_channel,
            "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_summary": summary,
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    lines = [
        "# Linux Validation Terminal Dispatch Terminal Verdict Publish Archive Report",
        "",
        (
            "- Terminal Verdict Publish Archive Status: "
            f"**{str(payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status']).upper()}**"
        ),
        (
            "- Terminal Verdict Publish Archive Decision: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision']}`"
        ),
        (
            "- Terminal Verdict Publish Archive Exit Code: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_exit_code']}`"
        ),
        (
            "- Should Archive: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_should_archive']}`"
        ),
        (
            "- Requires Manual Action: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_requires_manual_action']}`"
        ),
        (
            "- Should Page Owner: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_should_page_owner']}`"
        ),
        (
            "- Channel: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_channel']}`"
        ),
        (
            "- Upstream Terminal Verdict Publish Status: "
            f"`{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_status']}`"
        ),
        (
            "- Upstream Terminal Verdict Publish Channel: "
        ...
        "## Summary",
        "",
        f"- {payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_summary']}",
        "",
        "## Reason Codes",
    ]
...
def build_github_output_values(
    ...
    return {
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status": str(
            payload["linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status"]
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision": str(
            payload["linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision"]
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_exit_code": str(
            payload["linux_validation_terminal_dispatch_terminal_verdict_publish_archive_exit_code"]
        ),
        ...
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_channel": str(
            payload["linux_validation_terminal_dispatch_terminal_verdict_publish_archive_channel"]
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_publish_status": str(
            payload["linux_validation_terminal_dispatch_terminal_verdict_publish_status"]
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_publish_channel": str(
            payload["linux_validation_terminal_dispatch_terminal_verdict_publish_channel"]
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_follow_up_queue_url": str(
            payload["follow_up_queue_url"]
        ),
        ...
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_run_url": str(
            payload["release_run_url"]
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_reason_codes": json.dumps(
            payload["reason_codes"],
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_report_json": str(
            output_json
        ),
        "workflow_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_report_markdown": str(
            output_markdown
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Archive Linux validation terminal dispatch terminal verdict publish contract "
            "from P2-78 terminal verdict publish report"
        )
    )
    parser.add_argument(
        ...
        help="P2-78 Linux validation terminal dispatch terminal verdict publish report JSON path",
    )
    parser.add_argument(
        "--output-json",
        ...
        help="Output Linux validation terminal dispatch terminal verdict publish archive JSON path",
    )
    parser.add_argument(
        "--output-markdown",
        ...
        help="Output Linux validation terminal dispatch terminal verdict publish archive markdown report path",
    )
    parser.add_argument(
        "--github-output",
        default=None,
    ...
    parser.add_argument(
        "--print-report",
        action="store_true",
        help="Print Linux validation terminal dispatch terminal verdict publish archive JSON payload",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    terminal_verdict_publish_report_path = Path(
        args.linux_validation_terminal_dispatch_terminal_verdict_publish_report
    )
    ...
    try:
        terminal_verdict_publish_report = (
            load_linux_validation_terminal_dispatch_terminal_verdict_publish_report(
                terminal_verdict_publish_report_path
            )
        )
        payload = build_linux_validation_terminal_dispatch_terminal_verdict_publish_archive_payload(
            terminal_verdict_publish_report,
            source_path=terminal_verdict_publish_report_path.resolve(),
        )
    except ValueError as exc:
        print(
            "[p2-linux-ci-workflow-linux-validation-terminal-dispatch-terminal-verdict-publish-archive-gate] "
            f"{exc}",
            file=sys.stderr,
        )
        return 2
    ...
    if not args.dry_run:
        ...
        print(
            "linux validation terminal dispatch terminal verdict publish archive json: "
            f"{output_json_path}"
        )
        print(
            "linux validation terminal dispatch terminal verdict publish archive markdown: "
    ...
    print(
        "linux validation terminal dispatch terminal verdict publish archive summary: "
        "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status="
        f"{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_status']} "
        "linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision="
        f"{payload['linux_validation_terminal_dispatch_terminal_verdict_publish_archive_decision']} "
