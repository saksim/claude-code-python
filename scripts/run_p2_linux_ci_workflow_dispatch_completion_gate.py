"""Phase 2 card P2-28 gate for Linux CI workflow dispatch completion await.

This script consumes the P2-27 dispatch trace artifact and converges to a
final completion verdict:
1) validate trace contract and polling command,
...
TERMINAL_TRACE_STATUSES: set[str] = {
    "blocked",
    "ready_dry_run",
...
def load_dispatch_trace_report(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError(f"{path}: dispatch trace report not found")
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"{path}: dispatch trace payload must be object")
    ...
    tracking_status = _coerce_str(payload["tracking_status"], field="tracking_status", path=path)
    ...
    if tracking_status not in allowed_tracking_statuses:
        raise ValueError(
            f"{path}: field 'tracking_status' must be one of "
            f"{sorted(allowed_tracking_statuses)}"
        )

    trace_exit_code = _coerce_int(payload["trace_exit_code"], field="trace_exit_code", path=path)
    if trace_exit_code < 0:
        raise ValueError(f"{path}: field 'trace_exit_code' must be >= 0")
    ...
    poll_command = _coerce_str(payload["poll_command"], field="poll_command", path=path)
    poll_command_parts = _coerce_str_list(
        payload["poll_command_parts"],
        field="poll_command_parts",
        path=path,
    )
    poll_attempted = _coerce_bool(payload["poll_attempted"], field="poll_attempted", path=path)
    poll_returncode_raw = payload["poll_returncode"]
    if poll_returncode_raw is None:
        poll_returncode = None
    else:
        poll_returncode = _coerce_int(
            poll_returncode_raw,
            field="poll_returncode",
            path=path,
        )
    poll_status = _coerce_str(payload["poll_status"], field="poll_status", path=path)
    poll_conclusion = _coerce_str(payload["poll_conclusion"], field="poll_conclusion", path=path)
    poll_url = _coerce_str(payload["poll_url"], field="poll_url", path=path)
    poll_error_type = _coerce_str(payload["poll_error_type"], field="poll_error_type", path=path)
    poll_error_message = _coerce_str(
        payload["poll_error_message"],
        field="poll_error_message",
        path=path,
    )
    poll_stdout_tail = _coerce_str(payload["poll_stdout_tail"], field="poll_stdout_tail", path=path)
    poll_stderr_tail = _coerce_str(payload["poll_stderr_tail"], field="poll_stderr_tail", path=path)
    ...
    if should_poll_workflow_run:
        ...
        if len(poll_command_parts) < 3 or poll_command_parts[1:3] != ["run", "view"]:
            raise ValueError(
                f"{path}: poll_command_parts must include '<gh> run view ...' contract"
            )
        parsed_command = shlex.split(poll_command)
        if parsed_command != poll_command_parts:
    ...
    return {
        ...
        "execution_status": _coerce_str(payload["execution_status"], field="execution_status", path=path),
...
def build_dispatch_completion_payload(
    dispatch_trace_report: dict[str, Any],
    *,
    source_path: Path,
    project_root: Path,
    ...
    tracking_status = str(dispatch_trace_report["tracking_status"])
    should_poll_workflow_run = bool(dispatch_trace_report["should_poll_workflow_run"])

    completion_status = tracking_status
    completion_exit_code = int(dispatch_trace_report["trace_exit_code"])
    reason_codes = list(dispatch_trace_report["reason_codes"])

    poll_iterations = 0
    poll_attempted = False
    poll_returncode = dispatch_trace_report["poll_returncode"]
    poll_status = str(dispatch_trace_report["poll_status"])
    poll_conclusion = str(dispatch_trace_report["poll_conclusion"])
    poll_url = str(dispatch_trace_report["poll_url"])
    poll_error_type = str(dispatch_trace_report["poll_error_type"])
    poll_error_message = str(dispatch_trace_report["poll_error_message"])
    poll_stdout_tail = str(dispatch_trace_report["poll_stdout_tail"])
    poll_stderr_tail = str(dispatch_trace_report["poll_stderr_tail"])

    should_await = should_poll_workflow_run and tracking_status in {
        "run_tracking_ready",
        "run_in_progress",
    }
    ...
    if should_await and not dry_run:
        for attempt in range(1, max_polls + 1):
            ...
            poll_result = poll_executor(
                list(dispatch_trace_report["poll_command_parts"]),
                cwd=project_root,
            ...
            completion_exit_code = 0 if allow_in_progress else 1
            reason_codes.append("run_await_timeout")

    elif should_await and dry_run:
        completion_status = tracking_status
        completion_exit_code = int(dispatch_trace_report["trace_exit_code"])
        reason_codes.append("run_completion_poll_skipped_dry_run")
    else:
        if tracking_status == "run_completed_success":
            completion_exit_code = 0
        elif tracking_status in {"run_completed_failure", "run_tracking_missing", "run_poll_failed"}:
            completion_exit_code = 1
        elif tracking_status in {"blocked", "ready_dry_run"}:
            completion_exit_code = int(dispatch_trace_report["trace_exit_code"])

    if completion_status in TERMINAL_TRACE_STATUSES and completion_status == "run_in_progress":
        # Defensive fallback; run_in_progress should not be terminal.
        completion_exit_code = 0 if allow_in_progress else 1

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_dispatch_trace_report": str(source_path),
        "project_root": str(project_root),
        "execution_status": str(dispatch_trace_report["execution_status"]),
        "tracking_status": tracking_status,
        "completion_status": completion_status,
        "completion_exit_code": int(completion_exit_code),
        "dry_run": dry_run,
        "allow_in_progress": allow_in_progress,
        "should_poll_workflow_run": should_poll_workflow_run,
        "run_id": dispatch_trace_report["run_id"],
        "run_url": str(dispatch_trace_report["run_url"]),
        "repo_owner": str(dispatch_trace_report["repo_owner"]),
        "repo_name": str(dispatch_trace_report["repo_name"]),
        "workflow_path": str(dispatch_trace_report["workflow_path"]),
        "workflow_ref": str(dispatch_trace_report["workflow_ref"]),
        "poll_command": str(dispatch_trace_report["poll_command"]),
        "poll_command_parts": list(dispatch_trace_report["poll_command_parts"]),
        ...
        "poll_stdout_tail": poll_stdout_tail,
        "poll_stderr_tail": poll_stderr_tail,
        "trace_reason_codes": list(dispatch_trace_report["reason_codes"]),
        "reason_codes": _unique(reason_codes),
        "failed_checks": list(dispatch_trace_report["failed_checks"]),
        "structural_issues": list(dispatch_trace_report["structural_issues"]),
        "missing_artifacts": list(dispatch_trace_report["missing_artifacts"]),
        "source_dispatch_execution_report": str(dispatch_trace_report["source_dispatch_execution_report"]),
        "source_dispatch_report": str(dispatch_trace_report["source_dispatch_report"]),
    }


def render_markdown_report(payload: dict[str, Any]) -> str:
    lines = [
        "# Linux CI Workflow Dispatch Completion Report",
        "",
        f"- Completion Status: **{str(payload['completion_status']).upper()}**",
...
def build_github_output_values(
    ...
    return {
        "workflow_dispatch_completion_status": str(payload["completion_status"]),
        "workflow_dispatch_completion_exit_code": str(payload["completion_exit_code"]),
        "workflow_dispatch_completion_should_poll": (
            "true" if payload["should_poll_workflow_run"] else "false"
        ),
        "workflow_dispatch_completion_poll_attempted": (
            "true" if payload["poll_attempted"] else "false"
        ),
        "workflow_dispatch_completion_poll_iterations": str(payload["poll_iterations"]),
        "workflow_dispatch_completion_poll_status": str(payload["poll_status"]),
        "workflow_dispatch_completion_poll_conclusion": str(payload["poll_conclusion"]),
        "workflow_dispatch_completion_run_id": "" if run_id is None else str(run_id),
        "workflow_dispatch_completion_run_url": str(payload["run_url"]),
        "workflow_dispatch_completion_reason_codes": json.dumps(
            payload["reason_codes"],
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        "workflow_dispatch_completion_report_json": str(output_json),
        "workflow_dispatch_completion_report_markdown": str(output_markdown),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Await Linux CI workflow dispatch completion from P2-27 dispatch trace report"
    )
    parser.add_argument(
        "--dispatch-trace-report",
        default=".claude/reports/linux_unified_gate/ci_workflow_dispatch_trace.json",
        help="P2-27 dispatch trace report JSON path",
    )
    parser.add_argument(
        "--project-root",
        default=".",
    ...
    if args.poll_interval_seconds < 1:
        print(
            "[p2-linux-ci-workflow-dispatch-completion-gate] --poll-interval-seconds must be >= 1",
            file=sys.stderr,
        )
        return 2
    if args.max_polls < 1:
        print(
            "[p2-linux-ci-workflow-dispatch-completion-gate] --max-polls must be >= 1",
            file=sys.stderr,
        )
        return 2
    if args.poll_timeout_seconds < 1:
        print(
            "[p2-linux-ci-workflow-dispatch-completion-gate] --poll-timeout-seconds must be >= 1",
            file=sys.stderr,
        )
        return 2

    dispatch_trace_report_path = Path(args.dispatch_trace_report)
    output_json_path = Path(args.output_json)
    output_markdown_path = Path(args.output_markdown)
    project_root = Path(args.project_root).resolve()

    try:
        dispatch_trace_report = load_dispatch_trace_report(dispatch_trace_report_path)
        payload = build_dispatch_completion_payload(
            dispatch_trace_report,
            source_path=dispatch_trace_report_path.resolve(),
        ...
        print(f"[p2-linux-ci-workflow-dispatch-completion-gate] {exc}", file=sys.stderr)
        return 2
    ...
    if not args.dry_run:
        ...
        print(f"dispatch completion json: {output_json_path}")
        print(f"dispatch completion markdown: {output_markdown_path}")
    ...
    print(
        ...
        f"completion_exit_code={payload['completion_exit_code']}"
    )
    if args.print_report or args.dry_run:
        print(json.dumps(payload, indent=2, ensure_ascii=False))

    return int(payload["completion_exit_code"])
