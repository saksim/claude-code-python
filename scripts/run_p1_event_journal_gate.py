"""Phase 1 event journal regression gate entrypoint.

Runs the curated P1-02 event-journal runtime tests with one command.
"""
...
P1_EVENT_JOURNAL_TEST_FILES: tuple[str, ...] = (
    "tests/test_event_journal_runtime.py",
    "tests/test_server_runtime.py",
)


def build_p1_event_journal_pytest_command(
    *,
...
def _validate_manifest() -> list[str]:
    missing: list[str] = []
    for test_file in P1_EVENT_JOURNAL_TEST_FILES:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Phase 1 event-journal gate tests")
    parser.add_argument(
        "--python-executable",
    ...
    if missing:
        for path in missing:
            print(f"[p1-event-journal-gate] missing test file: {path}", file=sys.stderr)
        return 2
    ...
    command = build_p1_event_journal_pytest_command(
